...
// read from virtual memory in another process
size_t read_virt_mem(void* dst, void const* src, size_t size);

// read from virtual memory in another process
template <typename T>
T read_virt_mem(uint64_t cr3, uintptr_t src);

// read from virtual memory in another process
template <typename T>
T read_virt_mem(uintptr_t src);
...
// get the image size of the hypervisor
uint64_t get_hv_size();
...
// read from virtual memory in another process
inline size_t read_virt_mem(void* const dst, void const* const src,
	size_t const size) {
	return read_virt_mem(hv::g_cr3, dst, src, size);
}

// read from virtual memory in another process
template <typename T>
inline T read_virt_mem(uint64_t const cr3, uintptr_t src) {

    T buffer = {};
    read_virt_mem(cr3, &buffer, (void*)src, sizeof(T));
    return buffer;
}

// read from virtual memory in another process
template <typename T>
inline T read_virt_mem(uintptr_t src) {
	return read_virt_mem<T>(hv::g_cr3, src);
}
...
// get the image size of the hypervisor
inline uint64_t get_hv_size() {
	hv::hypercall_input input;
	input.code = hv::hypercall_get_hv_size;
	input.key = hv::hypercall_key;
	return hv::vmx_vmcall(input);
}

inline void* get_cpu_base() {
	hv::hypercall_input input;
	input.code = hv::hypercall_get_cpu_base;
	input.key = hv::hypercall_key;
	return reinterpret_cast<void*>(hv::vmx_vmcall(input));
}
...
inline uint64_t get_section_base_process(DWORD pid)
{
	hv::hypercall_input input;
	input.code = hv::hypercall_section_base;
	input.key = hv::hypercall_key;
	input.args[0] = pid;
	return hv::vmx_vmcall(input);
}

inline uint64_t scan_process_dtb(DWORD pid)
{
	hv::hypercall_input input;
